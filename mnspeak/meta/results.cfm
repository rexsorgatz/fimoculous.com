...
<cfif checkuser.recordcount greater than 0>
	...
	<cfelse>
		<cfset reason = "the password you\'ve typed is invalid. Please try again.">
	</cfif>
	
<!-- if the username was not present in the database -->	
<cfelse>
	<cfoutput>
	<cfset reason = 'there is no one named #form.userid# here.'>
	</cfoutput>
</cfif>
...
<cfif session.loggedin>
	...
	self.location ='/mnspeak/archive/post-<cfoutput>#url.entryid#</cfoutput>.cfm';
	<cfelseif isdefined("url.calnum")>
	self.location ='/mnspeak/calendar.cfm?calnum=<cfoutput>#url.calnum#</cfoutput>';
	<cfelseif isdefined("url.aggregator")>
	self.location ='/mnspeak/aggregator/index-personal.cfm';
	<cfelse>
	self.location ='http://www.mnspeak.com';
	</cfif>
	</script>
